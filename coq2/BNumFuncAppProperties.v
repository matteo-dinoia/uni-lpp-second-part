...
(* ****************************** *)
(* ****************************** *)
(* LEZIONE DEL 28/11/23. Hai commentato sin qui sopra.
Per le lezioni successive, potresti commentare alcune
delle dimostrazioni seguenti
*)
(* ****************************** *)
(* ****************************** *)
(* ****************************** *)
...
Proof. apply (BinNotation_fmap_homomorphism PN.pred PN.succ). Qed.
...
Lemma fmapIsStarPure: forall (a b: Set) (f: a -> b) (x: BinNotation a),
  fmap f x =  (pure f) <*> x.
Proof. destruct x.
- unfold fmap. unfold pure. unfold starApp. reflexivity.   
- unfold fmap. unfold pure. unfold starApp. reflexivity.   
Qed.

(* Proposition BinNotation_fmapVsPure: forall {a b: Set} (f: a -> b) (x: a),
   (pure ; (fmap f)) x = (f ; pure) x.
Proof. unfold pure. unfold fmap. reflexivity. Qed. *)
...
Proof. unfold pure. unfold starApp. 
destruct g. 
- reflexivity.
- reflexivity.
Qed.
